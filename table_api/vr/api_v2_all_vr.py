...
class APIv2AllVRRunner:
    """parallel execution of five VIP Roulette API environments' executor"""
    ...
    def run_all_parallel(self) -> Dict[str, Dict]:
        """
        parallel execution of all VIP Roulette API scripts

        Returns:
            execution result dictionary
        """
        logger.info(
            "Starting parallel execution of all VIP Roulette API scripts..."
        )
    ...
    def run_all_sequential(self) -> Dict[str, Dict]:
        """
        sequential execution of all VIP Roulette API scripts (for comparison)

        Returns:
            execution result dictionary
        """
        logger.info(
            "Starting sequential execution of all VIP Roulette API scripts..."
        )
    ...
    def print_summary(self):
        ...
        print("\n" + "=" * 80)
        print("VIP Roulette API v2 All-in-One Execution Summary")
        print("=" * 80)
    ...
    def save_outputs(self, output_dir: str = "vr_api_outputs"):
        """save all VIP Roulette scripts' outputs to file"""
        output_path = Path(output_dir)
        output_path.mkdir(exist_ok=True)

        for env_name, result in self.results.items():
            output_file = output_path / f"{env_name.lower()}_vr_output.txt"
            ...
            logger.info(
                f"Saved {env_name} VIP Roulette output to {output_file}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="parallel execution of five VIP Roulette API environments' scripts"
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--save-outputs",
        action="store_true",
        help="save all VIP Roulette scripts outputs to file",
    )
    parser.add_argument(
        "--output-dir",
    ...
    try:
        if args.mode == "parallel":
            logger.info("Running VIP Roulette APIs in PARALLEL mode")
            results = runner.run_all_parallel()
        else:
            logger.info("Running VIP Roulette APIs in SEQUENTIAL mode")
        ...
        if failed_count > 0:
            logger.warning(f"{failed_count} VIP Roulette script(s) failed")
            sys.exit(1)
        else:
            logger.info("All VIP Roulette scripts completed successfully!")

    except KeyboardInterrupt:
        logger.info("VIP Roulette execution interrupted by user")
        sys.exit(1)
    except Exception as e:
        logger.error(f"Unexpected error in VIP Roulette execution: {e}")
        sys.exit(1)
