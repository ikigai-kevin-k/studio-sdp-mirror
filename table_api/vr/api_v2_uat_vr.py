...
# UAT ARO-002
accessToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzZXNzaW9uSWQiOiIzODBlZDZkMS0xODk4LTRhYjMtYTk1My0zMjM3NmFkY2IyMDYiLCJnYW1lQ29kZSI6WyJBUk8tMDAyIl0sInJvbGUiOiJzZHAiLCJjcmVhdGVkQXQiOjE3NDk3OTIyNzA3ODAsImlhdCI6MTc0OTc5MjI3MH0.pEAeKqh_svk6FH_IL9O-WzN-9gzOdKZJ7wgZFnyVEuA"
...
def finish_post_v2_uat(url, token):
    headers = {
        "accept": "application/json",
        "Bearer": token,
        "x-signature": "los-local-signature",
...
def visibility_post_uat(url, token, enable):
    headers = {
        "accept": "application/json",
        "Bearer": token,
        "x-signature": "los-local-signature",
...
def get_roundID_v2_uat(url, token):
    # Set up HTTP headers
...
def pause_post_v2_uat(url, token, reason):
    headers = {
        "accept": "application/json",
        "Bearer": token,
        "x-signature": "los-local-signature",
...
def resume_pos_v2_uat(url, token):
    headers = {
        "accept": "application/json",
        "Bearer": token,
        "x-signature": "los-local-signature",
...
def get_sdp_config_v2_uat(url, token):
    """
...
def update_sdp_config_from_file_v2_uat(url, token, config_file="sdp.config"):
    """
    ...
    try:
        ...
        sdp_config_post_v2_uat(url, token, config_data)
        return True
...
def cancel_post_v2_uat(url: str, token: str) -> None:
    """
    取消當前局次
    """
    try:
...
def bet_stop_post_uat(url: str, token: str) -> bool:
    """
    Stop betting for the current round - Virtual Roulette game (UAT environment)
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            print(f"Error in bet_stop_post_uat: {error_msg}")
            return False
        ...
        print(f"Network error in bet_stop_post_uat: {e}")
        return False
    except ValueError as e:
        print(f"JSON decode error in bet_stop_post_uat: {e}")
        return False
    except Exception as e:
        print(f"Unexpected error in bet_stop_post_uat: {e}")
        return False


def broadcast_post_v2_uat(
