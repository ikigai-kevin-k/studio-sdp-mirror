...
class ARO001_2MockClient:
    ...
    async def connect(self):
        ...
        try:
            ...
            # Format: ?id=DEVICE_NAME&token=TOKEN&gameCode=TABLE_ID
            # Add suffix to match server broadcast pattern: ARO-001-2-*
            connection_id = f"{self.device_name}-backup"
    ...
    def start_main_vip(self):
        """Start main_vip.py process in tmux session 'sdp'."""
        try:
            # Get the home directory and construct the full path
            home_dir = os.path.expanduser("~")
            studio_dir = os.path.join(home_dir, "studio-sdp-roulette")
            main_vip_path = os.path.join(studio_dir, "main_vip.py")
            
            if not os.path.exists(main_vip_path):
                logger.error(f"❌ main_vip.py not found at {main_vip_path}")
                return False
            
            logger.info(f"🚀 Starting main_vip.py in tmux session 'sdp' from {studio_dir}")
            
            # Create tmux command to run main_vip.py in the 'sdp' session
            tmux_command = [
                "tmux", "send-keys", "-t", "sdp",
                f"cd {studio_dir} && sudo venv/bin/python main_vip.py",
                "Enter"
            ]
            ...
            logger.error(f"❌ Failed to start main_vip.py in tmux: {e}")
            return False

    def stop_main_vip(self):
        """Stop main_vip.py process in tmux session 'sdp'."""
        try:
            logger.info("🛑 Stopping main_vip.py in tmux session 'sdp'...")
            ...
            if result.returncode == 0:
                logger.info("✅ Stop command sent to tmux session 'sdp' successfully")
                return True
            else:
            ...
            logger.error(f"❌ Error stopping main_vip.py in tmux: {e}")
            return False
        ...
        try:
            ...
            if "data" in error_data and "signal" in error_data["data"]:
                ...
                if self.current_state == DeviceState.IDLE:
                    ...
                    # Start main_vip.py when transitioning to running state
                    logger.info("🎯 Starting main_vip.py due to error signal...")
                    self.start_main_vip()
