...
from ws_client import SmartStudioWebSocketClient, StudioServiceStatusEnum, StudioMaintenanceStatusEnum
...
async def update_sicbo_game_status(
    ...
    if _ws_client is not None and _table_id == table_id:
        try:
            ...
            all_accepted = all(field in preferences["accepted_fields"] for field in status_to_send.keys())
            
            if all_accepted:
                logger.info("🎯 All status updates were accepted successfully!")
            else:
                rejected_fields = [field for field in status_to_send.keys() if field not in preferences["accepted_fields"]]
            ...
            logger.warning(f"⚠️  Existing connection failed, will create new connection: {e}")
    ...
    client = SmartStudioWebSocketClient(SERVER_URL, table_id, DEVICE_NAME, TOKEN, fast_connect=fast_mode)
