...
# DEV ARO-002 (using access token from sr-1.json DEV environment)
# Note: This uses the same access token as sr-1.json DEV environment for ARO-001
# but configured for ARO-002 (VIP Roulette)
accessToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzZXNzaW9uSWQiOiIzZTcxOTlkYi0zYTlkLTRjZDYtYmUwZi04MGYzZThmYjhjNTQiLCJnYW1lQ29kZSI6WyJBUk8tMDAyIl0sInJvbGUiOiJzZHAiLCJjcmVhdGVkQXQiOjE3NjQwNDEwNzk3MDIsImlhdCI6MTc2NDA0MTA3OX0.BAFifyWYAngP3AB9Iq6baSPKjCLkQTC_2DBUk3sctVk"


def start_post_v2_dev(url, token):
    # Set up HTTP headers
    headers = {
...
def deal_post_v2_dev(url, token, round_id, result):
...
def finish_post_v2_dev(url, token):
    headers = {
        "accept": "application/json",
        "Bearer": token,
        "x-signature": "los-local-signature",
...
def visibility_post_dev(url, token, enable):
    headers = {
        "accept": "application/json",
        "Bearer": token,
        "x-signature": "los-local-signature",
...
def get_roundID_v2_dev(url, token):
    # Set up HTTP headers
...
def pause_post_v2_dev(url, token, reason):
    headers = {
        "accept": "application/json",
        "Bearer": token,
        "x-signature": "los-local-signature",
...
def resume_pos_v2_dev(url, token):
    headers = {
        "accept": "application/json",
        "Bearer": token,
        "x-signature": "los-local-signature",
...
def sdp_config_post_v2_dev(url, token, config_data):
    """
...
def get_sdp_config_v2_dev(url, token):
    """
    ...
    Returns:
        tuple: (broker_host, broker_port, room_id) from sdpConfig, or (None, None, None) if not found
    """
    headers = {
        "accept": "application/json",
        "Bearer": f"Bearer {token}",
...
def update_sdp_config_from_file_v2_dev(url, token, config_file="sdp.config"):
    """
    ...
    try:
        ...
        sdp_config_post_v2_dev(url, token, config_data)
        return True
...
def cancel_post_v2_dev(url: str, token: str) -> None:
    """
    取消當前局次
    """
    try:
...
def bet_stop_post_dev(url: str, token: str) -> bool:
    """
    Stop betting for the current round - Virtual Roulette game (DEV environment)
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            print(f"Error in bet_stop_post_dev: {error_msg}")
            return False
        ...
        print(f"Network error in bet_stop_post_dev: {e}")
        return False
    except ValueError as e:
        print(f"JSON decode error in bet_stop_post_dev: {e}")
        return False
    except Exception as e:
        print(f"Unexpected error in bet_stop_post_dev: {e}")
        return False
...
if __name__ == "__main__":
    ...
    while cnt < 1:
        ...
        round_id, status, betPeriod = get_roundID_v2_dev(get_url, token)
        ...
        print("================Deal================\n")
        time.sleep(18)
        bet_stop_post_dev(post_url, token)
        deal_post_v2_dev(post_url, token, round_id, results)
        print("================Finish================\n")
        finish_post_v2_dev(post_url, token)
