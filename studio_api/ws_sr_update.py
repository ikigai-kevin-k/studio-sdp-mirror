#!/usr/bin/env python3
"""
Single status update script for Roulette Game (ARO-001 table).
Imports functions from ws_sr.py for device status updates.
"""
...
# Add the current directory to Python path to import ws_sr
sys.path.append(os.path.dirname(__file__))

from ws_sr import test_aro_001_device_info
...
async def update_roulette_game_status(
    custom_status: Dict[str, Any] = None,
    table_id: str = "ARO-001",
    fast_mode: bool = True,
) -> bool:
    """
    Update Roulette game status with default values or custom status.
    ...
    # Default status for Roulette game
    default_status = {
        "maintenance": False,
        "zCam": "up",
        "sdp": "up",
...
async def main():
    ...
    logger.info("🎰 Roulette Game Single Status Update Test")
    logger.info("=" * 50)

    # Test 1: Use default status
    logger.info("\n📤 Test 1: Using default status...")
    success = await update_roulette_game_status()
