...
class StandbyClient:
    ...
    async def connect(self):
        ...
        try:
            ...
            # Connect as ARO-002-2-SDP to receive backup signals from ARO-002-2-SDP
    ...
    def start_main_vip_2(self):
        """Start main_vip_2.py process in tmux session 'dp' window 'sdp'."""
        try:
            # Get the home directory and construct the full path
            home_dir = os.path.expanduser("~")
            studio_dir = os.path.join(home_dir, "studio-sdp-roulette")
            main_vip_2_path = os.path.join(studio_dir, "main_vip_2.py")
            
            if not os.path.exists(main_vip_2_path):
                logger.error(f"❌ main_vip_2.py not found at {main_vip_2_path}")
                return False
            
            logger.info(f"🚀 Starting main_vip_2.py in tmux session 'dp' window 'sdp' from {studio_dir}")
            
            # Create tmux command to run main_vip_2.py in the 'dp:sdp' session/window
            tmux_command = [
                "tmux", "send-keys", "-t", "dp:sdp",
                f"sudo bash -c 'cd {studio_dir} && venv/bin/python3 main_vip_2.py'",
                "Enter"
            ]
            ...
            if result.returncode == 0:
                logger.info("✅ main_vip_2.py command sent to tmux session 'dp' window 'sdp' successfully")
                
                # Wait a moment for main_vip_2.py to start up
                import time
                time.sleep(2)
            ...
            logger.error(f"❌ Failed to start main_vip_2.py in tmux: {e}")
            return False

    def stop_main_vip_2(self):
        """Stop main_vip_2.py process in tmux session 'dp' window 'sdp'."""
        try:
            logger.info("🛑 Stopping main_vip_2.py in tmux session 'dp' window 'sdp'...")
            ...
            logger.error(f"❌ Error stopping main_vip_2.py in tmux: {e}")
            return False

    def check_main_vip_2_status(self):
        """Check if main_vip_2.py is still running in tmux session."""
        try:
            # Check if there's a Python process running main_vip_2.py
            check_command = [
            ...
            if result.returncode == 0:
                # Check if main_vip_2.py is still running by looking for process indicators
                output = result.stdout.lower()
                
                # Look for signs that main_vip_2.py has terminated
                ...
                for indicator in terminated_indicators:
                    if indicator in output:
                        logger.info(f"🔍 Detected main_vip_2.py termination: {indicator}")
                        return False
                ...
                if "$" in output and "main_vip_2.py" not in output:
                    logger.info("🔍 Detected shell prompt, main_vip_2.py likely terminated")
                    return False
            ...
            logger.error(f"❌ Error checking main_vip_2.py status: {e}")
    ...
    async def handle_error_signal(self, error_data: Dict[str, Any]):
        ...
        try:
            ...
            # Format 1: { signal: {...}, cmd: {} } - Direct signal from studioAPI server
            if "signal" in error_data:
                signal = error_data["signal"]
            # Format 2: { event: "exception", data: { signal: {...} } }
            elif "data" in error_data and "signal" in error_data["data"]:
            ...
            if signal:
                ...
                if self.current_state == DeviceState.IDLE:
                    ...
                    # Start main_vip_2.py when transitioning to running state
                    logger.info("🎯 Starting main_vip_2.py due to error signal...")
                    self.start_main_vip_2()
                    ...
                    # Already in running state, check if main_vip_2.py is still running
                    if not self.check_main_vip_2_status():
                        logger.info("🔄 main_vip_2.py not running, restarting due to error signal...")
                        self.start_main_vip_2()
                    ...
                    if message:
                        try:
                            ...
                            if (
                                ...
                                # Handle direct signal from server (format: {"signal": {...}, "cmd": {}})
                                await self.handle_error_signal(data)
                            # Also check for wrapped signal messages
                            elif (
                                isinstance(data, dict)
                                and "cmd" in data
                                and "signal" in data.get("cmd", {})
                            ):
                                # Handle wrapped signal from server
                                await self.handle_error_signal(data)
                            else:
                                logger.info(
        ...
        try:
            ...
            if self.error_signal_received:
                ...
                while self.running:
                    ...
                    # Check main_vip_2.py status every 10 seconds (every 2 status updates)
                    status_check_counter += 1
                    if status_check_counter >= 2:  # 10 seconds = 2 * 5 seconds
                        if not self.check_main_vip_2_status():
                            logger.info("🔄 main_vip_2.py terminated, transitioning back to IDLE state")
                            self.current_state = DeviceState.IDLE
                            self.error_signal_received = False
                            break
                        status_check_counter = 0
...
async def main():
    ...
    try:
        ...
        DEVICE_NAME = "SDP"  # Connect as ARO-002-2-SDP to receive backup signals
        TOKEN = config["token"]
