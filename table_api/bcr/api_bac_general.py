...
def load_config(env_name):
    """
    Load configuration from table-config-baccarat-v2.json

    Args:
        env_name (str): Environment name (CIT_BCR)
...
def update_sdp_config_from_file(
    url, token, accessToken, config_file="sdp.config"
):
    """
...
def bet_stop_post(url, token, accessToken):
    """
...
def run_game_cycle(env_name, result=None):
    """
    Run a complete game cycle for the specified environment

    Args:
        env_name (str): Environment name (CIT_BCR)
        result (list): Deal result (default: ["", "D2", "SQ", "S10", "H6", ""])
    """
    if result is None:
        result = ["", "D2", "SQ", "S10", "H6", ""]

    print(
        f"================Running Game Cycle for {env_name}================\n"
    )
    ...
    try:
        print("================Start================\n")
        start_post(post_url, token, accessToken)
        ...
        if round_id == -1:
            ...
            return
        print(
            f"Round ID: {round_id}, Status: {status}, Bet Period: {betPeriod}"
        )

        time.sleep(betPeriod + 1)
        # Stop betting
        print("================Bet Stop================\n")
        bet_stop_post(post_url, token, accessToken)
        ...
        print(
            f"================Game Cycle Completed for {env_name}================\n"
        )
...
if __name__ == "__main__":
    ...
    ENV = "CIT_BCR"  # Change environment as needed
    result = ["", "D2", "SQ", "S10", "H6", ""]  # Baccarat card result
