...
def load_config():
    ...
    try:
        with open(config_path, "r") as f:
            configs = json.load(f)
            # Find QAT-2 configuration
            for config in configs:
                if config["name"] == "QAT-2":
                    return config
        raise Exception("QAT-2 configuration not found in config file")
    except Exception as e:
        print(f"Error loading config: {e}")
        return None


# Load QAT-2 configuration
config = load_config()
if config:
    ...
    accessToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzZXNzaW9uSWQiOiJlZjQ4ODVlNy0zZDAzLTRmN2ItYjY3ZC01NzdmMjE4ODRmYzUiLCJnYW1lQ29kZSI6WyJBUk8tMDAxLTIiXSwicm9sZSI6InNkcCIsImNyZWF0ZWRBdCI6MTc1ODU5NTIzMDQ0NSwiaWF0IjoxNzU4NTk1MjMwfQ.hPCiPAXtp2JJVrqObwrWmTNtf2p340U04HkmlvZ7E2w"
    gameCode = "ARO-001-2"
