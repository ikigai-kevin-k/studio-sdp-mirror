...
SELF_TEST_POSITION_FILE = os.path.join(STUDIO_SDP_DIR, ".last_position_self_test_sync_speed.json")
...
def extract_today_logs_from_self_test() -> bool:
    """
    Extract today's logs from self-test-2api.log and create speed_{today}.log file
...
def sync_today_logs_from_self_test(today_log_file: str) -> int:
    """
    Sync today's logs from self-test-2api.log to speed_{today}.log file
    Only appends new logs that haven't been synced yet
    
    Args:
        today_log_file: Path to today's log file (speed_{today}.log)
    ...
    try:
        ...
        # Append today's logs to speed_{today}.log
...
def find_latest_log_file() -> Optional[str]:
    ...
    Priority: 1) speed_{today}.log (current day), 2) create it from self-test-2api.log if needed, 3) logs/sdp_serial.log (current), 4) latest speed_{yyyy-mm-dd}.log (legacy)
    ...
    # First, check if today's log file exists (speed_{today}.log)
    today_log_file = os.path.join(STUDIO_SDP_DIR, f"speed_{today}.log")
    if os.path.exists(today_log_file):
        ...
        if file_size < 100:
            ...
            print(f"   Note: Will monitor existing file. Use generate_today_speed_log.py to regenerate from logs/sdp_serial.log")
    ...
    # Third, fallback to current log file (main_speed.py writes here)
    if os.path.exists(CURRENT_LOG_FILE):
        return CURRENT_LOG_FILE
    
    # Fourth, fallback to date-split log files (legacy)
...
def read_new_log_lines(log_file_path: str) -> List[Dict]:
    ...
    try:
        ...
        # Check if saved position is too old (more than 2 hours)
...
def push_logs_to_loki(log_entries: List[Dict]) -> bool:
    ...
    try:
        ...
        # From error messages, Loki typically only accepts entries from the last 1-2 hours
        # To be safe, we'll only keep entries from the last 2 hours
        cutoff_time = datetime.now() - timedelta(hours=2)
