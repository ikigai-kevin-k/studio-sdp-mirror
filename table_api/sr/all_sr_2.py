#!/usr/bin/env python3
"""
API v2 All-in-One Script for Speed Roulette 2 (SR-2)
...
class APIv2AllSR2Runner:
    ...
    def save_outputs(self, output_dir: str = "sr2_api_outputs"):
        """save all Speed Roulette 2 scripts' outputs to file"""
        output_path = Path(output_dir)
        output_path.mkdir(exist_ok=True)

        for env_name, result in self.results.items():
            output_file = output_path / f"{env_name.lower().replace('-', '_')}_sr2_output.txt"
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="sr2_api_outputs",
        help="output file directory (default: sr2_api_outputs)",
    )

    args = parser.parse_args()

    runner = APIv2AllSR2Runner()
