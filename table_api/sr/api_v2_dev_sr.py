...
def load_config():
    ...
    try:
        with open(config_path, "r") as f:
            configs = json.load(f)
            # Find DEV configuration
            for config in configs:
                if config["name"] == "DEV":
                    return config
        raise Exception("DEV configuration not found in config file")
...
def visibility_post_v2_dev(url, token, enable):
    headers = {
        "accept": "application/json",
        "Bearer": token,
        "x-signature": "los-local-signature",
...
def resume_post_v2_dev(url, token):
    headers = {
        "accept": "application/json",
        "Bearer": token,
        "x-signature": "los-local-signature",
...
def bet_stop_post_dev(url: str, token: str) -> bool:
    """
    Stop betting for the current round - Speed Roulette game (DEV environment)
...
def _get_broadcast_metadata_dev(broadcast_type, signal_type="warning"):
    """
...
def broadcast_post_v2_dev(
    ...
    broadcast_data = _get_broadcast_metadata_dev(broadcast_type, signal_type="warning")
