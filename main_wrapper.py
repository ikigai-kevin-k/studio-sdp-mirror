    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            game_config = GameConfig(
                ...
                room_id=self.config.get("room", {}).get("id")
                or self.config.get("room_id"),
                broker_host=self.config.get("broker", {}).get("host")
                or self.config.get("broker_host"),
                broker_port=self.config.get("broker", {}).get("port")
                or self.config.get("broker_port"),
            ...
            if not self.controller:
                raise Exception(
                    f"Failed to create controller for game type: {self.game_type}"
                )
