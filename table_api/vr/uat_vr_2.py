...
def load_config():
    ...
    try:
        with open(config_path, "r") as f:
            configs = json.load(f)
            # Find UAT-2 configuration
            for config in configs:
                if config["name"] == "UAT-2":
                    return config
        raise Exception("UAT-2 configuration not found in config file")
    except Exception as e:
        print(f"Error loading config: {e}")
        return None


# Load UAT-2 configuration
config = load_config()
if config:
    ...
    accessToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzZXNzaW9uSWQiOiJjMDc0YTA4YS03MTkzLTQ1NzktODE5Ni00NzlkOWIxMjJjYmUiLCJnYW1lQ29kZSI6WyJBUk8tMDAyLTIiXSwicm9sZSI6InNkcCIsImNyZWF0ZWRBdCI6MTc1ODY5NTczODUxNSwiaWF0IjoxNzU4Njk1NzM4fQ.n5qClEXT8nqdXGF9x654L26uFNyErguanDhOeES39wM"
    gameCode = "ARO-002-2"
