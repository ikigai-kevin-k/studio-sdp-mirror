...
def load_config():
    ...
    try:
        with open(config_path, "r") as f:
            configs = json.load(f)
            # Find PRD-3 configuration
            for config in configs:
                if config["name"] == "PRD-3":
                    return config
        raise Exception("PRD-3 configuration not found in config file")
    except Exception as e:
        print(f"Error loading config: {e}")
        return None


# Load PRD-3 configuration
config = load_config()
if config and "access_token" in config:
    ...
    accessToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzZXNzaW9uSWQiOiI2ZTYxN2RjNS1kN2NkLTQwOGUtYTE1MS1hYjc2ZWNmNGZjZjciLCJnYW1lQ29kZSI6WyJBUk8tMDAzIl0sInJvbGUiOiJzZHAiLCJjcmVhdGVkQXQiOjE3NjA0Mjg4NzgyNjUsImlhdCI6MTc2MDQyODg3OH0.IcAosh7GRzPzcPznPBBTCCmtCD9LusZv9nXJsN1f_ZI"
    gameCode = "ARO-003"
