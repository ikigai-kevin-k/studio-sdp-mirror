...
# GLC ARO-002
accessToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzZXNzaW9uSWQiOiI4ZDA2NDEzNC0zNmExLTQ1Y2YtOGFiYi1jYzJlNzJhYWMxMWMiLCJnYW1lQ29kZSI6WyJBUk8tMDAyIl0sInJvbGUiOiJzZHAiLCJjcmVhdGVkQXQiOjE3NjI1MDcyMjc3ODEsImlhdCI6MTc2MjUwNzIyN30.jsbvoC8RdxH8hbHij8jFGaklXOqeggiB8bTUM_OXIJY"


def start_post_v2_glc(url, token):
    # Set up HTTP headers
    headers = {
...
def deal_post_v2_glc(url, token, round_id, result):
...
def finish_post_v2_glc(url, token):
    headers = {
        "accept": "application/json",
        "Bearer": token,
        "x-signature": "los-local-signature",
...
def visibility_post_glc(url, token, enable):
    headers = {
        "accept": "application/json",
        "Bearer": token,
        "x-signature": "los-local-signature",
...
def get_roundID_v2_glc(url, token):
    # Set up HTTP headers
...
def pause_post_v2_glc(url, token, reason):
    headers = {
        "accept": "application/json",
        "Bearer": token,
        "x-signature": "los-local-signature",
...
def resume_pos_v2_glc(url, token):
    headers = {
        "accept": "application/json",
        "Bearer": token,
        "x-signature": "los-local-signature",
...
def sdp_config_post_v2_glc(url, token, config_data):
    """
...
def get_sdp_config_v2_glc(url, token):
    """
...
def update_sdp_config_from_file_v2_glc(url, token, config_file="sdp.config"):
    """
    ...
    try:
        ...
        sdp_config_post_v2_glc(url, token, config_data)
        return True
...
def cancel_post_v2_glc(url: str, token: str) -> None:
    """
    取消當前局次
    """
    try:
...
def bet_stop_post_glc(url: str, token: str) -> bool:
    """
    Stop betting for the current round - Virtual Roulette game (GLC environment)
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            print(f"Error in bet_stop_post_glc: {error_msg}")
            return False
        ...
        print(f"Network error in bet_stop_post_glc: {e}")
        return False
    except ValueError as e:
        print(f"JSON decode error in bet_stop_post_glc: {e}")
        return False
    except Exception as e:
        print(f"Unexpected error in bet_stop_post_glc: {e}")
        return False
...
if __name__ == "__main__":
    ...
    while cnt < 1:
        ...
        # get_url =  "https://crystal-los.iki-glc.cc/v1/service/table/"
        # post_url = "https://crystal-los.iki-glc.cc/v1/service/sdp/table/"
        ...
        round_id, status, betPeriod = get_roundID_v2_glc(get_url, token)
        ...
        print("================Deal================\n")
        time.sleep(18)
        bet_stop_post_glc(post_url, token)
        deal_post_v2_glc(post_url, token, round_id, results)
        print("================Finish================\n")
        finish_post_v2_glc(post_url, token)
