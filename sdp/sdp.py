...
import websockets
import argparse
# import analogist as ao, 改成MQTT控制, 所以不需要
# import generator as gen, 改成MQTT控制, 所以不需要 

# Plan: launch mqtt_sdp_client.py, 
# check是否跟mqtt-los-client.py連線成功
# check是否透過mqtt可以跟idp連線
# check是否透過mqtt可以跟diceshaker連線
...
class TestRouletteSimulator(RouletteSimulator):
    def __init__(self, recorder_port=8765, room_id=1):
        ...
        # 這邊也要繼承一些MQTT client的屬性


    # Hearbeat跟跟RR的toggle connection是作為自我監控的機制    
    # 但toggle connection也要在MQTT client中實作, 以作為websocket的備援
...
class DiceShaker:
    # TODO: DiceShaker跟Roulette要抽象出一個Game的bass class
    """骰子震動器控制類別"""
    ...
    # TODO: start_shaking中替換成MQTT, deprecieated
...
# TODO: 會被替換成main-original-idp.py中的main game loop
# (Optional)
