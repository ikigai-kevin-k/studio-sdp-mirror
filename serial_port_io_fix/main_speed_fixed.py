...
# -*- coding: utf-8 -*-
"""
Fixed version of main_speed.py with improved serial communication
...
# Initialize serial connection with improved settings
def create_improved_serial_connection():
    """Create serial connection with improved settings"""
    try:
        ser = serial.Serial(
            port="/dev/ttyUSB1",
            baudrate=9600,
            ...
            inter_byte_timeout=0.01,  # Inter-byte timeout
        )
        ...
        print(f"Failed to create serial connection: {e}")
        return None


ser = create_improved_serial_connection()
...
# Create data queue for non-blocking processing
...
def read_from_serial():
    """Improved serial reading with non-blocking approach"""
...
def process_serial_data():
    """Process serial data from queue (separate thread)"""
    global x2_count, x5_count, last_x2_time, last_x5_time, start_post_sent, deal_post_sent, start_time, deal_post_time, finish_post_time, isLaunch, sensor_error_sent
...
def main():
    ...
    process_thread = threading.Thread(target=process_serial_data)
