...
def load_config():
    ...
    try:
        with open(config_path, "r") as f:
            configs = json.load(f)
            # Find STG configuration
            for config in configs:
                if config["name"] == "STG":
                    return config
        raise Exception("STG configuration not found in config file")
    except Exception as e:
        print(f"Error loading config: {e}")
        return None


# Load STG configuration
config = load_config()
if config:
...
def bet_stop_post_stg(url: str, token: str) -> bool:
    """
    Stop betting for the current round - Speed Roulette game (STG environment)
...
def _get_broadcast_metadata_stg(broadcast_type, signal_type="warning"):
    """
...
def broadcast_post_v2_stg(
    ...
    broadcast_data = _get_broadcast_metadata_stg(broadcast_type, signal_type="warning")
