#!/usr/bin/env python3
"""
Split self-test-2api.log by date for Speed Roulette
Reads self-test-2api.log and splits it into speed_{yyyy}-{mm}-{dd}.log files
...
def find_last_speed_log_date() -> Optional[datetime]:
    """Find the date of the last existing speed_*.log file"""
    import glob
    pattern = "speed_*.log"
    ...
    for log_file in log_files:
        ...
        match = re.search(r'speed_(\d{4}-\d{2}-\d{2})\.log', filename)
        if match:
            try:
...
def split_log_by_date(input_file: str = "self-test-2api.log", output_prefix: str = "speed_") -> None:
    """
    ...
    Args:
        ...
        output_prefix: Prefix for output files (default: "speed_")
    """
    ...
    last_date = find_last_speed_log_date()
    ...
    if last_date:
        ...
        print(f"📅 No existing speed log files found, processing all dates up to {today}")
        last_date = None
...
def main():
    """Main function"""
    print("=" * 60)
    print("Speed Roulette Log Splitter by Date (Incremental)")
    print("=" * 60)
    ...
    if not os.path.exists(input_file):
        print(f"❌ Error: File '{input_file}' not found!")
        print("\nUsage:")
        print("   python split_speed_log_by_date.py [input_file]")
        print("\nDefault input file: self-test-2api.log")
        return
    
    split_log_by_date(input_file, "speed_")
