...
def visibility_post_v2_uat(url, token, enable):
    headers = {
        "accept": "application/json",
        "Bearer": token,
        "x-signature": "los-local-signature",
...
def resume_post_v2_uat(url, token):
    headers = {
        "accept": "application/json",
        "Bearer": token,
        "x-signature": "los-local-signature",
...
def bet_stop_post_uat(url: str, token: str) -> bool:
    """
    Stop betting for the current round - Speed Roulette game (UAT environment)
...
def _get_broadcast_metadata_uat(broadcast_type, signal_type="warning"):
    """
...
def broadcast_post_v2_uat(
    ...
    broadcast_data = _get_broadcast_metadata_uat(broadcast_type, signal_type="warning")
