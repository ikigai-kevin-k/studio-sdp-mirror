...
def calculate_time_intervals(entries):
    ...
    for i in range(1, len(entries)):
        ...
        intervals.append((prev_time, current_time, time_diff, prev_error, current_error))
...
def plot_interval_distribution(intervals, output_file="interval_distribution.png"):
    ...
    plt.axvline(np.mean(interval_values), color='red', linestyle='dashed', linewidth=1, label=f'Mean: {np.mean(interval_values):.2f}s')
    plt.axvline(np.median(interval_values), color='green', linestyle='dashed', linewidth=1, label=f'Median: {np.median(interval_values):.2f}s')
...
def plot_error_timeline(entries, output_file="error_timeline.png"):
    ...
    color_map = {error: plt.cm.tab10(i/len(unique_errors)) for i, error in enumerate(unique_errors)}
...
def main(log_file_path):
    ...
    print(f"Shortest time interval: {min_interval[2]:.2f} seconds (between {min_interval[0]} and {min_interval[1]})")
    print(f"Longest time interval: {max_interval[2]:.2f} seconds (between {max_interval[0]} and {max_interval[1]})")
    ...
    try:
        ...
        print("If you need chart functionality, please make sure matplotlib is installed (pip install matplotlib)")
