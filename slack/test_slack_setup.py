...
def test_webhook_connection():
    ...
    try:
        ...
        response = client.send(text="🧪 測試訊息 - 如果你看到這則訊息，設定就成功了！")
...
def test_bot_token():
    ...
    try:
        ...
        if response["ok"]:
            ...
            print(f"❌ Bot Token 驗證失敗: {response.get('error', 'Unknown error')}")
            return False
...
def test_user_token():
    ...
    try:
        ...
        if response["ok"]:
            ...
            print(f"❌ User Token 驗證失敗: {response.get('error', 'Unknown error')}")
            return False
...
def test_slack_notifier_functionality():
    ...
    try:
        from slack_notifier import SlackNotifier, send_error_to_slack, send_success_to_slack
