...
Calculates game_duration_aro22 as the sum of all four time intervals
Specifically for ARO-002-2 (ARO22) instance
"""
...
# Log file path (same as main_vip_2.py uses)
LOG_FILE = "time_intervals-2api.log"

# Instance label for ARO-002-2
INSTANCE_LABEL = "aro22"
...
STATE_FILE = os.path.join(os.path.dirname(__file__), ".last_metrics_aro22.json")

# State file to track last read position in log file
POSITION_FILE = os.path.join(os.path.dirname(__file__), ".last_position_aro22.json")
...
def parse_metrics_from_block(block: str):
    ...
    Returns:
        dict: Dictionary containing the four metrics plus game_duration_aro22, or None if parsing failed
    """
    metrics = {}
...
def send_metrics_to_prometheus(metrics: dict, instance_label: str = INSTANCE_LABEL):
    ...
    Args:
        metrics: Dictionary containing the four metrics plus game_duration_aro22
        instance_label: Instance label for the metrics (default: aro22)
    ...
    game_duration.labels(instance=instance_label).set(metrics['game_duration_aro22'])
    ...
    try:
        ...
        print(f"  - game_duration_aro22: {metrics['game_duration_aro22']:.4f}s")
...
def monitor_log_file(log_file_path: str):
    ...
    print(f"   Instance: {INSTANCE_LABEL} (ARO-002-2)")
    ...
    try:
        while running:
            ...
            for metrics in new_metrics_list:
                ...
                # Send metrics to Prometheus with ARO22 instance label
...
def main():
    ...
    print("=" * 60)
    print("Prometheus Duration Metrics Monitor - ARO-002-2 (ARO22)")
    print("=" * 60)
    ...
    print(f'  game_duration_aro22{{job="{JOB_NAME}", instance="{INSTANCE_LABEL}"}}')
